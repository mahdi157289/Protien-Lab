    ...
    return (
      <div className="w-full px-4 py-12">
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-2 xl:grid-cols-3 animate-pulse">
          {[...Array(6)].map((_, idx) => (
            <SkeletonProductCard key={`offer-skel-${idx}`} />
          ))}
        </div>
      </div>
    );
      ...
      >
        ...
        >
          {duplicatedOffers.map((offer, index) => {
            ...
            return (
              <motion.div
                key={offer.__k || `offer-${index}`}
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: (index % offers.length) * 0.1 }}
                ...
                <div className="p-6 font-source-sans">
                  ...
                  <div className="mb-4">
                    <div className="flex items-center gap-3 mb-1">
                      <span className="text-2xl font-normal text-green-600 font-source-sans">
                        {offer.newPrice.toFixed(2)} TD
                      </span>
                      <span className="text-lg text-gray-400 line-through font-source-sans">
