...
import { SkeletonFilterPanel, SkeletonProductCard } from "../common/Skeletons";
...
const useProductFilters = (initialProducts) => {
  ...
  const applyFilters = (products) => {
    ...
    if (filters.brands && filters.brands.length > 0) {
      const selectedBrands = normalizeValues(filters.brands);
      const selectedSet = new Set(selectedBrands);
      filtered = filtered.filter((product) => {
        const productBrand = product.brand ? product.brand.toString().toLowerCase().trim() : '';
        const productNameRaw = product.name ? product.name.toString().toLowerCase() : '';
        const productNameNoSpace = productNameRaw.replace(/\s+/g, '');

        const brandMatch = productBrand && selectedSet.has(productBrand);
        const nameMatch = selectedBrands.some((b) => {
          if (!b) return false;
          const brandRaw = b;
          const brandNoSpace = b.replace(/\s+/g, '');
          return productNameRaw.includes(brandRaw) || productNameNoSpace.includes(brandNoSpace);
        });
...
function ProductList() {
  ...
  useEffect(() => {
    ...
    setFilters((prev) => {
      const next = { ...prev };
      next.brands = brandName ? [brandName] : [];
      next.categories = categoryName ? [categoryName] : [];
      return next;
    });
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await api.get('/users/products?limit=500');
        ...
        console.log('🛒 Loaded products for store page:', {
          receivedType: Array.isArray(response.data) ? 'array' : typeof response.data,
          count: list.length,
  ...
  // Fetch brands from "Nos Marque" photos (same as admin side)
  useEffect(() => {
    const fetchBrands = async () => {
      ...
      try {
        ...
        if (response.data.success && response.data.data) {
          // Extract unique brand names from photos (same logic as admin)
          ...
          // Fallback: extract from products if photo fetch fails
  ...
  }, [products]); // Include products as fallback dependency
    ...
    return (
      <div className="min-h-screen px-8 py-10 bg-black backdrop-blur-sm">
        <div className="mb-8 text-center">
          <div className="h-8 w-48 mx-auto bg-primary/20 rounded animate-pulse" />
          <div className="h-4 w-64 mx-auto mt-3 bg-primary/10 rounded animate-pulse" />
        </div>
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
          ...
          <div className="lg:col-span-9">
            <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-3 animate-pulse">
              {[...Array(9)].map((_, idx) => (
                <SkeletonProductCard key={`product-skel-${idx}`} />
              ))}
            </div>
          </div>
        </div>
  ...
  return (
    <motion.div 
      ...
      className="min-h-screen px-8 py-10 bg-black backdrop-blur-sm"
    >
      ...
      >
        <motion.button 
          ...
          className="w-full sm:w-[420px] h-[60px] bg-green-500 hover:bg-green-600 font-bold border-2 border-white rounded-[50px] transition duration-200"
        ...
        >
          {filteredProducts.length === 0 ? (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="p-8 text-center rounded-2xl bg-secondary text-white border border-primary/10 shadow-xl"
            >
              <p className="text-lg text-gray-400">{t('product_no_products')}</p>
              <button
                onClick={resetFilters}
            ...
            <motion.div
              ...
              className="grid grid-cols-1 gap-12 px-0 sm:grid-cols-2 lg:grid-cols-3 justify-items-center"
            >
              <AnimatePresence>
